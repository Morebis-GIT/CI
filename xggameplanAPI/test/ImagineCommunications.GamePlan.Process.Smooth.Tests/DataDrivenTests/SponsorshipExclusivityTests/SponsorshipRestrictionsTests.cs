...
namespace ImagineCommunications.GamePlan.Process.Smooth.Tests
{
        ...
        : DataDrivenTests
    {
        ...
        private static readonly char[] _pipeSeperator = new[] { '|' };
        private static readonly char[] _colonSeperator = new[] { ':' };
            ...
            return new Sponsorship
            {
                ...
                SponsoredItems = new List<SponsoredItem>{
                    new SponsoredItem{
                        ...
                        Products = sponsoredProduct.Split(_pipeSeperator, StringSplitOptions.None),
            ...
            string expectedBreaks)
        {
            ...
            {
                firstExpectedBreaks = expectedBreaks.Split(_pipeSeperator, StringSplitOptions.None)[0];
            }
            ...
            string code = null)
        {
            ...
            ref readonly ProgrammeResult[] expectedResult =
                ref SmoothFixture.RecommendationsAreSetWithNoSponsorshipRulesExpectedResults();
            ...
            _ = pivotSalesArea.ProgrammesExternalReference
                    .Should()
                    .BeEquivalentTo(expectedProgrammesExternalReference, becauseArgs: null);
            ...
            foreach (var result in expectedResult)
            {
                ...
                _ = pivotProgramme.BreaksExternalReference
                        .Should()
                        .BeEquivalentTo(result.Breaks.Select(x => x.ExternalReference), testCode);
                ...
                foreach (var aBreak in result.Breaks)
                {
                    ...
                    for (int position = 0; position < aBreak.SpotsInPositionOrder.Length; position++)
                    {
                        string spot = aBreak.SpotsInPositionOrder[position];

                        _ = pivotBreak.Spot(spot)
                            .ShouldBePlaced()
                            .AtActualPositionInBreak(position + 1);
                    }
                }
            }
        }
        ...
        {
            var pivotRecommendations = recommendations.Pivot(RepositoryFactory);
            var expectedBreakArray = expectedBreaks.Split(_pipeSeperator, StringSplitOptions.None);
            var competitorsArray = competitors.Split(_pipeSeperator, StringSplitOptions.None);
            var sponsoredProductArray = sponsoredProduct.Split(_pipeSeperator, StringSplitOptions.None);
            ...
            string competitors)
        {
            ...
            foreach (var competitorValuePair in competitors.Split(_pipeSeperator, StringSplitOptions.None))
            {
                string competitor;
                int? value;
                ...
                {
                    string[] valuePair = competitorValuePair.Split(_colonSeperator, StringSplitOptions.None);
